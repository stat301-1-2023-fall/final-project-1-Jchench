...
One variable I was particularly interested in looking at was `final_streams` as an indication popularity because it measures how many times a song is played by users of Spotify. I wanted to see if there were any features that would make a song more/ less popular so I looked at correlation coefficients:
...
In terms of categorical variables, I think the ones that are most interesting to look at would be genre, key, and time signature. Moving forwards, we should look at the distribution of `final_streams`, `year`, `dance`, `loud`, `primary_genre`, `key`, `time_signature`, and `song_seconds`. We can take a closer look at these variables in the univariate and bivariate analyses.
...
![Dancebility here is measure in a percentage value for how "danceable" a song is. This histogram seems to have more or less a unimodal normal distribution with the exception of a couple outliers below 25 percent.](plots/dance.png)
...
spotify_data |> 
  ...
  labs(title = "Total Number of Spotify Streams vs. Loudness",
       ...
       y = "total streams")
```
We can also see here that there is a relatively weak positive correlation between the total number of Spotify streams for a song and its loudness. This means that songs that are louder tend to be played somewhat more often by Spotify users.
...
```
As shown in the plot above, it looks like song duration is also negatively correlated with year. This is quite interesting because it could also show a shift in song preference trends over time.
...
spotify_data |> 
  ...
  labs("Total Number of Spotify Streams vs. Song Key",
       ...
       y = "total streams")
```
It looks like songs that are in the A#/Bb minor and G minor keys have the highest mean number of total streams. Songs in the A minor and D#/Eb major have the lowest mean number of total streams. Songs in the F#/Gb minor key seem to have the widest range and interquartile range. Songs in D#/Eb major also have the smallest range.
