...
One potential issue I'm going to face in terms of merging is having to remove/ work around missing values. Since I'm thinking of merging by song and artist name, it will be a bit hard since a couple of song names in the `spotify_streams` dataset is missing. Another challenge I will face will be regarding the `date` and `time` variables in the `spotify_data_top` dataset. I might have to mutate these rows so that it shows the year, month, day separately. I may also need to find a better way to represent `time` numerically. I also need to rename a lot of the columns because the variable names are not quite clear as to what they're representing. For example, the `time` variable actually represents the length of the song in minutes and seconds. Another issue that that the `spotify_data_top` dataset does not have any information on songs from 2021, ^[the Kaggle user who provided this dataset could not find any information from 2021] which will impact my analysis going forward if I decide to look at songs across time. Overall, the datasets seem relatively clean. 

## Misc

**Potential timeline:** I'm hoping to have my data fully cleaned by the end of next week so I can start doing some initial exploration.
